...
class GPUVecEnv(VecEnv):
    def __init__(
        ...
        self.jax_single_step = jax.jit(
            jax.vmap(lambda m, d: mjx.step(m, d), in_axes=(None, 0))
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        self.action_delay = (
            round(self.action_delay / self.actual_timestep) * self.actual_timestep
        )
        self.joint_observation_delay = random.uniform(
            MIN_DELAY * self.randomization_factor, MAX_DELAY * self.randomization_factor
        )
        self.joint_observation_delay = (
            round(self.joint_observation_delay / self.actual_timestep)
            * self.actual_timestep
        )
        self.local_ang_vel_delay = random.uniform(
            MIN_DELAY * self.randomization_factor, MAX_DELAY * self.randomization_factor
        )
        self.local_ang_vel_delay = (
            round(self.local_ang_vel_delay / self.actual_timestep)
            * self.actual_timestep
        )
        ...
        self.torso_local_velocity_delay = (
            round(self.torso_local_velocity_delay / self.actual_timestep)
            * self.actual_timestep
        )
        ...
        self.torso_local_accel_delay = (
            round(self.torso_local_accel_delay / self.actual_timestep)
            * self.actual_timestep
        )
        ...
        self.local_gravity_vector_delay = (
            round(self.local_gravity_vector_delay / self.actual_timestep)
            * self.actual_timestep
        )
        ...
        self.pressure_values_delay = (
            round(self.pressure_values_delay / self.actual_timestep)
            * self.actual_timestep
        )

        # make buffers for observations and actions
        self.action_buffer = [self.data_batch.ctrl] * (int)(
            self.action_delay / self.actual_timestep
        )
        ...
        for i in range((int)(self.joint_observation_delay / self.actual_timestep)):
            self.joint_angles_buffer.append(
                jp.array([[0] * len(JOINT_NAMES)] * self.num_envs)
            )
        for i in range((int)(self.joint_observation_delay / self.actual_timestep)):
            self.joint_velocities_buffer.append(
                jp.array([[0] * len(JOINT_NAMES)] * self.num_envs)
            )
        ...
        for i in range((int)(self.local_gravity_vector_delay / self.actual_timestep)):
            self.local_gravity_vector_buffer.append(
                jp.array([self.gravity_vector] * self.num_envs)
            )
        for i in range((int)(self.pressure_values_delay / self.actual_timestep)):
            self.pressure_values_buffer.append(
                jp.array([[0] * len(PRESSURE_GEOM_NAMES)] * self.num_envs)
            )
    ...
    def _get_obs(self):
        ...
        joint_angles = self.data_batch.qpos[:, self.joint_qpos_idx] + (
            self.randomization_factor
        ...
        joint_velocities = self.data_batch.qvel[:, self.joint_dof_idx] + (
            self.randomization_factor
        ...
        torso_local_accel = (
            (torso_local_velocity - self.previous_torso_local_velocity)
            / self.actual_timestep
        ) + (
            self.randomization_factor
