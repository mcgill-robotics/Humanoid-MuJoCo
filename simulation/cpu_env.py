...
class CPUEnv(gym.Env):
    ...
    def _init_sim_trackers(self):
        self.action_to_take = self.data.ctrl
        self.lastAction = jp.zeros(self.data.ctrl.shape)
        self.lastLastAction = jp.zeros(self.data.ctrl.shape)
    ...
    def _get_obs(self):
        ...
        torso_z_pos = jp.array([torso_z_pos])
    ...
    def _apply_action(self, action):
        ...
        self.action_to_take = self.action_buffer.pop(0)
        # actions should be inputted to the environment in the -1 to 1 range, and they are mapped here to -pi/2 and pi/2 accordingly
        self.action_to_take = jp.clip(jp.array(self.action_to_take), -1, 1)
        self.action_to_take = self.action_to_take * (jp.pi / 2)
        self.data.ctrl = self.action_to_take
    ...
    def step(self, action=None):
        ...
        self.lastAction = jp.expand_dims(self.action_to_take, axis=0)
...
if __name__ == "__main__":
    sim = CPUEnv(
        xml_path=SIM_XML_PATH, reward_fn=rewardControlInputFn, randomization_factor=1
    )
    obs = sim.reset()
