...
namespace mjpc::humanoid_cap {

std::string Stand::XmlPath() const {
  return GetModelPath("humanoid_cap/stand/task.xml");
}
std::string Stand::Name() const { return "Humanoid Cap Stand"; }
...
void Stand::ResidualFn::Residual(const mjModel* model, const mjData* data,
  ...
  double* f1_position = SensorByName(model, data, "right_bottom_foot");
  double* f2_position = SensorByName(model, data, "left_bottom_foot");
  double* head_position = SensorByName(model, data, "torso_position");
  double head_feet_error =
      head_position[2] - 0.5 * (f1_position[2] + f2_position[2]);
