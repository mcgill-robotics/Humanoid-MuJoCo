...
class GPUBatchSimulation:
  ...
  def reset(self):
    ...
    self.data_batch = jax.vmap(lambda rng: self.base_mjx_data.replace(qpos=jax.random.uniform(rng, self.base_mjx_data.qpos.shape, minval=-1.0*self.randomization_factor*JOINT_INITIAL_STATE_OFFSET_MAX/180.0*jp.pi, maxval=self.randomization_factor*JOINT_INITIAL_STATE_OFFSET_MAX/180.0*jp.pi)))(self.rng)
