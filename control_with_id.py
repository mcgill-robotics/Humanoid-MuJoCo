...
# Constants
TORSO_ORIENTATION_TARGET = np.array(
    [0, 0, 0]
...
def compute_optimal_joint_targets(
    ...
    current_orientation = pin.rpy.matrixToRpy(
        data.oMf[model.getFrameId("torso")].rotation
    )
    ...
    h = np.concatenate(
        [
            model.upperPositionLimit[7:] - joint_positions,
            joint_positions - model.lowerPositionLimit[7:],
        ]
    )

    # Solve the QP problem
    joint_accelerations = solve_qp(P, q, G, h, solver="cvxopt")

    # Compute the new joint positions
    dt = 1/50  # Assuming a control frequency of 100 Hz
    new_joint_positions = (
        joint_positions + joint_velocities * dt + 0.5 * joint_accelerations * dt**2
    )
...
if __name__ == "__main__":
    ...
    while True:
        ...
        while True:
            ...
            local_gravity_direction = obs[
                27:30
            ]  # unit vector which points in the direction of the force of gravity, relative to the torso of the robot
